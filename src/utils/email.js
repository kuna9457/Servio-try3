...
transporter.verify((error, success) => {
  if (error) {
    ...
    console.error('3. Used the App Password in EMAIL_PASSWORD environment variable');
    console.error('4. No spaces around the = sign in your .env file');
  } else {
    console.log('SMTP server is ready to send emails');
  }
});

// Add this function after the transporter verification
...
export const sendVerificationEmail = async (email, code) => {
  try {
    if (!validateEmailConfig()) {
      throw new Error('Company email is not configured in environment variables');
    }
